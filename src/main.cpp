...
#if defined(NDEBUG)
# error "Oceancoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9e3a3a8aaa6fc0c8a22040bea0f717a3e8fa0cb083bddfd81c2a811db4b16f39");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Oceancoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Oceancoin Signed Message:\n";
...
{
    // Oceancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
 ...
 for (int Loop = 0; Loop < nHeight/100; Loop++)
	{
...
static const int64 nTargetTimespan = 30 * 60; // Oceancoin: 30 minutes
static const int64 nTargetSpacing = 10 * 60; // Oceancoin: 10 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Oceancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Oceancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9e3a3a8aaa6fc0c8a22040bea0f717a3e8fa0cb083bddfd81c2a811db4b16f39");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This coin create to save the ocean. 05.06.17 : 14.05 GMT";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1496671500;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1922625;
        ...
        assert(block.hashMerkleRoot == uint256("0xe54bbf1fc328d77d4622ce9e1e72b4e7b13fd493c8a5c338202a226558fbc024"));
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc0, 0xa6, 0xdb }; // Oceancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OceancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OceancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OceancoinMiner(CWallet *pwallet)
{
    printf("OceancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("oceancoin-miner");
    ...
    try { loop {
        ...
        printf("Running OceancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OceancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OceancoinMiner, pwallet));
}
